...
class Workout extends Component {
    ...
    // TO DO: update this function to populate the database with the new information.
    // should save all the data in selectedCheckboxes. 
            ...
            db.collection("users").doc(uid).get().then((doc) => {
                ...
                if (this.state.date in workout_hist){
                    ...
                    for (var i = 0; i < workouts.length; i++){
                        var sets = workouts[i].sets;
                        // console.log("Sets " + sets);
                        var reps = workouts[i].reps;
                        var weight = workouts[i].weight;
                        console.log(`id: [${workouts[i].id}]`);
                        db.collection("workouts").doc(workouts[i].id).get().then((doc2) => {
                            let workoutData = doc2.data();
                            console.log("------ workout object -----");
                            console.log(workoutData);
                            console.log("---------------------------");
                            this.updateWorkoutState(workoutData.name);
                ...
                if (typeof recurringWorkouts != 'undefined') {
                    for (let i=0; i<recurringWorkouts.length; i++) {
                        db.collection("workouts").doc(recurringWorkouts[i].id).get().then((doc3) => {
                            this.updateRecurringState(doc3.data().name);
                        })
                    }
                }
            })}
                    ...
                    for (var i = 0; i < dailyWorkouts.length; i++) {
                        if (dailyWorkouts[i].completed) {
                            ...
                            db.collection("workouts").doc(dailyWorkouts[i].id).get().then((doc3) => {
                                ...
                                this.selectedCheckboxes.add(doc3.data().name);
